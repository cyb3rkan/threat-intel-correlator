...
Two pieces of durable, security-relevant state:

- **Audit log** — `paths.audit_log_path` (default `audit.log`) plus any rotated
  segments `audit.log.1 .. audit.log.N`. This is the tamper-evident hash chain;
  back it up to preserve the integrity record. Each rotated segment is an
  independent, self-verifiable chain (rotation seals the prior chain), so older
  segments remain verifiable on their own.
- **SQLite cache** — `paths.cache_dir/tic-cache.sqlite`. A performance cache of
  provider enrichments; reconstructible by re-running sweeps, so lower priority
  than the audit log.

### Backup procedure

The store is plain files; a filesystem snapshot or copy suffices. Quiesce
writers (no sweep in flight), or accept that the most recent in-flight record
may be excluded:
...
dst="backup/tic-${ts}"
mkdir -p "${dst}"
cp -a "${TIC_AUDIT_LOG_PATH:-audit.log}"* "${dst}/" 2>/dev/null || true
cp -a "${TIC_CACHE_DIR:-cache}/tic-cache.sqlite" "${dst}/" 2>/dev/null || true
tic audit verify --path "${dst}/audit.log"
```

### Restore procedure

Stop all writers, restore in place, and re-verify:

```sh
cp -a backup/tic-<ts>/audit.log* "$(dirname "${TIC_AUDIT_LOG_PATH:-audit.log}")/"
cp -a backup/tic-<ts>/tic-cache.sqlite "${TIC_CACHE_DIR:-cache}/"
tic audit verify
```

A `verify` failure after restore means the chain was truncated or tampered:
treat the restored log as suspect and fall back to an earlier known-good backup.

### Retention & purge

- Audit rotation is config-driven (`audit.max_bytes`, `audit.max_rotations`;
  default: never rotate). Sized rotation bounds disk use while retaining the
  most recent `max_rotations` sealed segments.
- Expired cache rows can be purged on demand with `tic cache purge`, or on every
  cache build by setting `cache_purge_on_startup: true` (default off, to keep
  default behavior unchanged and avoid double-purging with the on-demand command).

### RPO / RTO targets

Recommended targets for a local-first single-node deployment; tune to your
backup cadence:

- **RPO (Recovery Point Objective): ≤ 24h.** With a daily backup, at most one
  day of audit records and cache warmth is lost. Because the cache is
  reconstructible, the effective data-loss exposure is the audit records since
  the last backup; increase backup frequency for a tighter audit RPO.
- **RTO (Recovery Time Objective): ≤ 15min.** Recovery is a file copy plus
  `tic audit verify` — no database rebuild, no clustering to reconcile. The
  dominant cost is locating the latest known-good backup.

### Repository layout note

The project root is currently nested
(`…/threat-intel-correlator/threat-intel-correlator/`). Tooling and the
remediation bootstrap locate the root by a sentinel file
(`src/tic/infra/config.py`) and are nesting-agnostic, so this does not affect
operation. To flatten for packaging, `git mv` the inner project up and update CI
working-directory paths; this is a manual, history-preserving step intentionally
left out of the automated bootstrap.
...
- **UI adapter test coverage.** The Streamlit app (`src/tic/ui/app.py`)
  has been removed; the UI adapter (`src/tic/ui/adapter.py`) is tested
  via unit tests in `tests/unit/test_ui_adapter.py`. Full functional
  UI testing against the Next.js frontend is handled in CI.
