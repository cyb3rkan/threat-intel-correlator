...
class SafeHttpClient:
    def __init__(
        ...
        *,
        ...
        allowed_host_cidrs: frozenset[str] = frozenset(),
        ...
        self._allowed_cidrs = allowed_host_cidrs
    ...
    async def aclose(self) -> None:
        ...
        Must be called on the same event loop the client was created on.
        The FastAPI path satisfies this because run_sweep_async reuses the
        server's event loop throughout the request lifetime.

        The CLI/Streamlit path still calls asyncio.run(_run_async(...))
        which creates a fresh loop per sweep — aclose() is called inside
        the same asyncio.run() block (in the finally of _run_async), so
        the loop is still alive when we get here.

        If somehow called after the loop has been closed (e.g. a future
        refactor introduces a second asyncio.run), the RuntimeError is
        logged rather than silently swallowed so the leak becomes visible.
        """
        if self._closed:
            return
        self._closed = True
        try:
            ...
            # Log rather than silently discard — a RuntimeError here means
            # the client is being closed on the wrong loop, which is a bug
            # that would cause a connection pool leak.
            _log.warning("http_client_close_loop_mismatch", error=str(exc)[:120])
            return
    ...
    async def _request(
        ...
        ensure_public_url(
            url, extra_allowlist=self._extra, allowed_cidrs=self._allowed_cidrs
        )
        original_url = url
