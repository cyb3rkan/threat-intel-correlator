...
from tic.adapters.parsers.json_depth import check_json_depth
...
def parse_stix_feed(
    ...
    *,
    ...
    with resolved.open("r", encoding="utf-8", errors="strict") as f:
        text = f.read()
    check_json_depth(text, limits.max_json_depth)
    try:
        doc = json.loads(text)
        ...
        raise ParseError(
            "stix json nesting exceeds safe decode depth",
            user_message="STIX JSON nesting is too deep.",
        ) from e
