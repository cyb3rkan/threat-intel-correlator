...
from tic.adapters.parsers.json_depth import json_max_depth
    ...
    *,
    ...
    with resolved.open("r", encoding="utf-8", errors="strict") as f:
        for line_num, raw_line in enumerate(f, start=1):
            ...
            if not line:
                continue
            if json_max_depth(line) > limits.max_json_depth:
                _log.warning(
                    "ndjson_line_too_deep",
                    line=line_num,
                    depth_limit=limits.max_json_depth,
                )
                continue
            try:
                obj = json.loads(line)
                ...
                _log.warning("ndjson_line_nesting_too_deep", line=line_num)
                continue
