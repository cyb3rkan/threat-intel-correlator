...
from tic.adapters.secrets.env_store import EnvSecretStore
...
def build_secret_store() -> SecretStore:
    """Select the secret backend from ``TIC_SECRET_BACKEND`` (default: keyring).

    Local/dev keeps the OS keyring. Container/Kubernetes deployments set
    ``TIC_SECRET_BACKEND=env`` to read secrets from injected environment
    variables, because the keyring is unusable under a read-only root
    filesystem with no D-Bus session. Unknown values fail closed.
    """
    backend = os.environ.get("TIC_SECRET_BACKEND", "keyring").strip().lower()
    if backend in ("", "keyring"):
        return KeyringSecretStore()
    if backend == "env":
        return EnvSecretStore()
    raise ConfigError(
        f"unknown TIC_SECRET_BACKEND: {backend!r}",
        user_message="TIC_SECRET_BACKEND must be 'keyring' (default) or 'env'.",
    )
...
def load_audit_signing_key(
    ...
    """Return the audit HMAC key when signing is enabled, else None.

    Fail-closed: when ``audit.sign`` is true but the key is missing (or the
    secret backend errors), raise ConfigError rather than silently degrading
    to an unsigned chain. Callers use this on the WRITE path.
    """
    if not settings.audit.sign:
        return None
    try:
        return secret_store.get(
            settings.audit.keyring_service, settings.audit.keyring_user
        )
    except Exception as e:  # noqa: BLE001 — fail closed; never log the value
        raise ConfigError(
            "audit signing enabled but key missing from keyring",
            user_message=(
                "Audit signing is enabled but no key is configured. "
                "Provision one with `tic config set-key audit-hmac`."
            ),
        ) from e


def try_load_audit_hmac(settings: Settings, secret_store: SecretStore | None) -> bytes | None:
    """Best-effort load of the audit HMAC key for the VERIFY path.

    Returns None when the key is absent or the backend errors. ``verify_chain``
    fails closed on its own if a signed record is present but no key was loaded.
    """
    if secret_store is None:
        return None
    try:
    ...
    except Exception:  # noqa: BLE001 — log nothing; verify decides fail-closed
        return None


def build_cache(settings: Settings) -> Cache:
    db = settings.paths.cache_dir / "tic-cache.sqlite"
    cache = SqliteCache(db, allowed_root=settings.paths.cache_dir)
    if settings.cache_purge_on_startup:
        cache.purge_expired()
    return cache
...
def _build_one(
    ...
    *,
    ...
    extra_cidrs = (
        frozenset(cfg.allowed_host_cidrs) if cfg.allowed_host_cidrs else frozenset()
    )
    ...
    if not cfg.verify_tls:
        ...
        if audit is not None:
            ...
            try:
                ...
                _log.warning(
                    "audit_append_failed",
                    failed_event="provider_tls_verify_disabled",
                    error=type(e).__name__,
                )
    http = SafeHttpClient(
        http_cfg,
        extra_host_allowlist=extra,
        allowed_host_cidrs=extra_cidrs,
