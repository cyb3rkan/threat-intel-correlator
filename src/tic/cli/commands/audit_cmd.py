...
from tic.cli._wiring import build_secret_store, try_load_audit_hmac
...
def verify() -> None:
    ...
    try:
        ...
        signing_key = try_load_audit_hmac(settings, build_secret_store())
        ok = HashChainAuditLogger(
            settings.paths.audit_log_path, signing_key=signing_key
        ).verify_chain()
        if ok:
