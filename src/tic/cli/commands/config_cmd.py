...
def show(
    ...
    try:
        ...
        for name, cfg in settings.providers.items():
            typer.echo(
                f"  {name}:  enabled={cfg.enabled}  "
                f"keyring={cfg.keyring_service}/{cfg.keyring_user}"
            )
            if cfg.endpoint:
...
def _read_secret() -> str:
    if sys.stdin.isatty():
        secret: str = typer.prompt("Secret", hide_input=True, confirmation_prompt=False)
        return secret
...
def _resolve_target(provider: str) -> tuple[str, str]:
    ...
    if provider == "audit-hmac":
        return settings.audit.keyring_service, settings.audit.keyring_user
    ...
    try:
        ...
        if not yes:
            try:
                ...
                typer.echo("Aborted.", err=True)
                raise typer.Exit(code=int(ExitCode.SUCCESS)) from None
        try:
