...
app = typer.Typer(add_completion=False, help="Inspect and manage configuration and credentials.", no_args_is_help=True)
...
@app.command("show")
def show(verbose: bool = typer.Option(False, "--verbose", help="Show full endpoint/host values.")) -> None:
    ...
    try:
        ...
        for name, cfg in settings.providers.items():
            typer.echo(f"  {name}:  enabled={cfg.enabled}  keyring={cfg.keyring_service}/{cfg.keyring_user}")
            if cfg.endpoint:
        ...
        if settings.ai.endpoint_allowlist:
            typer.echo(f"  endpoints: {[_mask(e, verbose) for e in settings.ai.endpoint_allowlist]}")
        ...
        typer.echo(e.user_message, err=True); raise typer.Exit(code=e.exit_code) from e
    except Exception as e:  # noqa: BLE001
        typer.echo("An unexpected error occurred.", err=True); raise typer.Exit(code=int(ExitCode.INTERNAL_ERROR)) from e
...
def _resolve_target(provider: str) -> tuple[str, str]:
    ...
    raise ConfigError(f"unknown provider: {provider}", user_message=f"Unknown provider '{provider}'.")
...
def set_key(provider: str = typer.Argument(...)) -> None:
    ...
    try:
        ...
        secret = _read_secret()
        if not secret:
            typer.echo("Empty secret rejected.", err=True); raise typer.Exit(code=int(ExitCode.INPUT_ERROR))
        try:
            keyring.set_password(service, user, secret)
        except KeyringError as e:
            typer.echo("Failed to store secret in keyring.", err=True); raise typer.Exit(code=int(ExitCode.AUTH_ERROR)) from e
...
def delete_key(provider: str = typer.Argument(...), yes: bool = typer.Option(False, "--yes")) -> None:
    ...
    try:
        ...
        if not yes:
            try:
                if not typer.confirm(f"Delete keyring entry service='{service}' user='{user}'?", default=False):
                    typer.echo("Aborted.", err=True); raise typer.Exit(code=int(ExitCode.SUCCESS))
            except typer.Abort:
                typer.echo("Aborted.", err=True); raise typer.Exit(code=int(ExitCode.SUCCESS))
        try:
            keyring.delete_password(service, user)
        except KeyringError as e:
            typer.echo("Could not delete (may not exist).", err=True); raise typer.Exit(code=int(ExitCode.AUTH_ERROR)) from e
