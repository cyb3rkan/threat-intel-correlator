"""UI-framework-independent adapter for `tic sweep`.

Reuses the existing wiring + orchestrator + parsers + log source + renderers.
This module has no UI-framework dependency; its consumers are the FastAPI
backend (`tic.api.main`) and the CLI. It exposes only public-safe data so any
front end can render results without touching raw log lines or provider
payloads.
...
def _new_upload_target(
    *,
    upload_dir: Path,
    working_dir: Path,
    original_filename: str | None,
) -> Path:
    """Return a path-guarded, UUID-named target path inside upload_dir.

    The original filename is discarded except for its extension, which is only
    preserved if it is on the SAFE_EXTENSIONS allowlist. Does NOT create the
    file — callers either write bytes (`stage_upload`) or stream into it
    (`open_staged_path`). The path is validated with safe_resolve_within
    against working_dir.
    """
...
def open_staged_path(
    *,
    upload_dir: Path,
    working_dir: Path,
    original_filename: str | None,
) -> Path:
    """Allocate a path-guarded, empty upload target for streaming writes.

    Unlike stage_upload, this never materialises the payload in memory: the
    caller (e.g. the FastAPI layer) streams the request body straight to this
    path in bounded chunks. Returns the resolved target path; the file does
    not yet exist.
    """
    return _new_upload_target(
...
def stage_upload(
    ...
    *,
    ...
    In-memory variant retained for the CLI/Streamlit callers that already hold
    the bytes. The FastAPI layer streams to disk via open_staged_path instead.
    The returned path is re-validated with safe_resolve_within against
    working_dir.
    """
    target = _new_upload_target(
        upload_dir=upload_dir,
        working_dir=working_dir,
        original_filename=original_filename,
    )
...
async def _run_async(req: SweepRequest, settings: Settings) -> SweepResult:
    ...
    audit = HashChainAuditLogger(
        settings.paths.audit_log_path,
        signing_key=load_audit_signing_key(settings, build_secret_store()),
        max_bytes=settings.audit.max_bytes,
        max_rotations=settings.audit.max_rotations,
    )
    audit.append(
        "ui_invoke",
    ...
    try:
        ...
        orchestrator = SweepOrchestrator(
            ...
            provider_concurrency=settings.orchestrator.provider_concurrency,
            sweep_deadline_seconds=settings.orchestrator.sweep_deadline_seconds,
        )
...
async def run_sweep_async(req: SweepRequest, settings: Settings | None = None) -> SweepResult:
    """Async entry point for the FastAPI layer.

    Runs on the caller's event loop (no nested asyncio.run), so httpx
    clients are created and closed on the same loop — preventing the
    cross-loop RuntimeError that caused aclose() to silently fail and
    leak connection pool memory.
    """
    if settings is None:
        settings = get_settings()
    try:
        return await _run_async(req, settings)
    except TICError as e:
        _log.warning("ui_tic_error", type=type(e).__name__)
        raise RuntimeError(e.user_message) from None
    except Exception as e:  # noqa: BLE001
