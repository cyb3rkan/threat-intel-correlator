...
class ScoringProfile(BaseModel):
    ...
    `extra="forbid"`: a shipped/loaded profile YAML with unknown keys must
    fail loud at validation time rather than have those keys silently dropped.
    Only `version`, `weights`, and `severity_thresholds` are consumed by the
    scoring engine (`compute_score` / `severity_for_score`).
    """
