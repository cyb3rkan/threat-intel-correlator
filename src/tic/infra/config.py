...
class ParserLimits(BaseModel):
    # 64 MB default keeps the in-memory feed buffer reasonable on dev machines.
    # Operators handling large feeds can raise this via TIC_PARSER_LIMITS__* env vars.
    max_file_size_bytes: int = Field(default=64 * 1024 * 1024, ge=1024, le=10_737_418_240)
    max_json_depth: int = Field(default=64, ge=4, le=256)
    max_string_length: int = Field(default=8192, ge=64, le=65536)
    # 100 k IOCs keeps list(iocs) in orchestrator below ~50 MB for typical IOC sizes.
    max_iocs_per_feed: int = Field(default=100_000, ge=1, le=100_000_000)
...
class ProviderConfig(BaseModel):
    ...
    # Optional operator-declared CIDR ranges. When non-empty, an allowlisted
    # host's resolved IP must fall inside one of these ranges or the SSRF guard
    # rejects the request (closes DNS-rebind of an allowlisted host to a
    # different internal IP, e.g. a cloud metadata endpoint). When empty,
    # allowlisted hosts keep the documented residual (IP not range-checked).
    allowed_host_cidrs: list[str] = Field(default_factory=list, max_length=16)
    verify_tls: bool = True

    @field_validator("allowed_host_cidrs")
    @classmethod
    def _validate_allowed_host_cidrs(cls, v: list[str]) -> list[str]:
        out: list[str] = []
        for cidr in v:
            try:
                net = ipaddress.ip_network(cidr, strict=False)
            except ValueError as e:
                raise ValueError(f"invalid allowed_host_cidrs entry: {cidr}") from e
            out.append(str(net))
        return out
...
class AuditConfig(BaseModel):
    ...
    sign: when true, each audit record is signed with a chained HMAC-SHA256
    using a key from the keyring (service/user below). When false (the safe
    default), the log keeps only its SHA-256 link chain -- still tamper-evident
    against reordering/edits, but recomputable by anyone with write access.

    Provision the key with `tic config set-key audit-hmac`. With sign: true and
    no key present, the audit logger fails closed (never an unsigned fallback).
    """

    sign: bool = False
    keyring_service: str = "tic-audit-hmac"
    keyring_user: str = "default"

    # Retention: roll the log at max_bytes (None = never roll, the default, so
    # behavior is unchanged) and keep at most max_rotations sealed segments.
    # Each segment is an independent hash chain (rotation seals the prior chain).
    max_bytes: int | None = Field(default=None, ge=4096)
    max_rotations: int = Field(default=5, ge=1, le=100)


class OrchestratorConfig(BaseModel):
    """Sweep concurrency + time budget.

    provider_concurrency bounds the total in-flight enrichment calls across all
    IOCs (one global semaphore). sweep_deadline_seconds is an optional overall
    budget; when it elapses, IOCs still being enriched are returned without
    enrichment (partial-but-valid) and the sweep is flagged in the audit log.
    """

    provider_concurrency: int = Field(default=4, ge=1, le=64)
    sweep_deadline_seconds: float | None = Field(default=None, gt=0)
...
class Settings(BaseSettings):
    ...
    # Scheduled, config-driven purge of expired cache rows on cache build.
    # Default off: preserves behavior and avoids double-purging with the
    # explicit `tic cache purge` command. Set true to enable scheduled purge.
    cache_purge_on_startup: bool = False
    ...
    if config_file is not None and Path(config_file).exists():
        try:
            ...
            with open(config_file, encoding="utf-8") as _f:
                _doc: dict[str, Any] = _yaml.safe_load(_f) or {}
