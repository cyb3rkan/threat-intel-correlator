...
export default function DashboardPage() {
  ...
  // Lazy initialiser hydrates from localStorage on the client (loadHistory is
  // SSR-safe: it returns [] when window is undefined, so server and first
  // client render agree — no hydration mismatch, no setState-in-effect).
  const [history, setHistory] = React.useState<RunHistoryEntry[]>(loadHistory);
  ...
  React.useEffect(() => {
    ...
    // Flip the loading flag off the render pass (React 19 purity rules); the
    // fetch that follows drives it back to false when it settles.
    queueMicrotask(() => {
      if (alive) setProviderStatusLoading(true);
    });
  ...
  async function handleRun(input: SweepFormInput) {
    ...
    try {
      ...
      setResult(r);
      // A new result lands: start the user fresh. Done here (the single
      // place `result` changes) rather than in a derived-state effect, so
      // there is no setState-in-effect cascade (React 19 purity rules).
  ...
  // Guard against a stale selection referencing a finding that no longer
  // exists (e.g. a new result set). Derived during render instead of via a
  // setState-in-effect: if the selected id isn't among the current findings,
  // treat the selection as empty. `handleRun` also clears selectedId when a
  // new result lands, so this is a belt-and-suspenders read-time guard.
  const safeSelectedId =
    selectedId !== null && findings.some((f) => f.finding_id === selectedId)
      ? selectedId
      : null;

  const selected = React.useMemo(
    () => findings.find((f) => f.finding_id === safeSelectedId) ?? null,
    [findings, safeSelectedId],
  );
