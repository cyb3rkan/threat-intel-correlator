...
def _settings(tmp_path):
    return Settings(paths=PathsConfig(
        working_dir=tmp_path, cache_dir=tmp_path,
...
def log_match(tmp_path):
    ...
    p.write_text(json.dumps({"@timestamp": "2025-01-01T00:00:00Z", "src_ip": "1.2.3.4"}) + "\n", encoding="utf-8")
    return p
...
def log_no_match(tmp_path):
    ...
    p.write_text(json.dumps({"@timestamp": "2025-01-01T00:00:00Z", "src_ip": "8.8.8.8"}) + "\n", encoding="utf-8")
    return p
...
def _patches(tmp_path):
    return [
        ...
        patch("tic.cli.commands.sweep.try_load_redaction_hmac", return_value=b"unit-test-hmac-key-32-bytes-padded!!"),
    ]
...
def test_match_produces_terminal_output(tmp_path, feed, log_match):
    with _patches(tmp_path)[0], _patches(tmp_path)[1], _patches(tmp_path)[2], \
         _patches(tmp_path)[3], _patches(tmp_path)[4], _patches(tmp_path)[5], \
         _patches(tmp_path)[6]:
        result = runner.invoke(sweep_cmd.app,
            ["--feed", str(feed), "--logs", str(log_match), "--format", "terminal", "--fail-on", "critical"])
...
def test_json_output_parseable(tmp_path, feed, log_match):
    with _patches(tmp_path)[0], _patches(tmp_path)[1], _patches(tmp_path)[2], \
         ...
         _patches(tmp_path)[6]:
        result = runner.invoke(sweep_cmd.app,
            ["--feed", str(feed), "--logs", str(log_match), "--format", "json", "--fail-on", "critical"])
    assert result.exit_code == 0, result.output
...
def test_no_match_exits_zero(tmp_path, feed, log_no_match):
    with _patches(tmp_path)[0], _patches(tmp_path)[1], _patches(tmp_path)[2], \
         ...
         _patches(tmp_path)[6]:
        result = runner.invoke(sweep_cmd.app,
            ["--feed", str(feed), "--logs", str(log_no_match), "--format", "json", "--fail-on", "high"])
...
def test_fail_on_info_exits_one(tmp_path, feed, log_match):
    with _patches(tmp_path)[0], _patches(tmp_path)[1], _patches(tmp_path)[2], \
         ...
         _patches(tmp_path)[6]:
        result = runner.invoke(sweep_cmd.app,
            ["--feed", str(feed), "--logs", str(log_match), "--format", "json", "--fail-on", "info"])
    assert result.exit_code == 1


def test_invalid_format_exits_config_error(tmp_path, feed, log_no_match):
    result = runner.invoke(sweep_cmd.app,
        ["--feed", str(feed), "--logs", str(log_no_match), "--format", "avro"])
...
def test_summary_mode_truncates_ioc(tmp_path, feed, log_match):
    with _patches(tmp_path)[0], _patches(tmp_path)[1], _patches(tmp_path)[2], \
         ...
         _patches(tmp_path)[6]:
        result = runner.invoke(sweep_cmd.app,
            ["--feed", str(feed), "--logs", str(log_match),
             "--format", "json", "--fail-on", "critical", "--output-mode", "summary"])
    assert result.exit_code == 0
...
def test_hash_mode_uses_hmac_prefix(tmp_path, feed, log_match):
    with _patches(tmp_path)[0], _patches(tmp_path)[1], _patches(tmp_path)[2], \
        ...
        result = runner.invoke(sweep_cmd.app,
            ["--feed", str(feed), "--logs", str(log_match),
             "--format", "json", "--fail-on", "critical", "--output-mode", "hash"])
    assert result.exit_code == 0
    f = json.loads(result.stdout)["findings"][0]
...
def test_hash_mode_without_hmac_key_exits_with_friendly_error(
    tmp_path, feed, log_match
):
